...
function UploadDocModal({
  ...
  const {
    ...
    loading,
    error,
  } = useQuery(GET_PREVIOUS_MODULE_REPORT, {
    client,
    variables: {
      where: {
        id: currentComplain,
  ...
  // MUTATIONS
  const [updateModuleReportFn, updateStatus, resetFn] = useMutation(
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadedFilesData) {
        ...
        setUploading(false);
        toast.error("Something went wrong, please try again later 0000000");
      }
    } catch (error) {
