...
const ProductTable = ({ IndustryData, deleteIndustry }: IProductTableProps) => {
    ...
    return (
        <table className="w-full leading-normal">
            ...
            <tbody>
                {
                    IndustryData && IndustryData?.map((item, i) =>
                        <tr key={i}>
                            ...
                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-xs">
                                <div className="relative flex items-center justify-center  space-x-4 px-8 ">
                                    ...
                                    <Link href={`/admin/dashboard/settings/industry/details/${item?.id}`} className="focus:ring-2 focus:ring-offset-2  text-sm leading-none text-green-600 py-2 px-2 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none"><AiFillEye /></Link>
                                    <button onClick={() => deleteIndustry(item.id)} className="focus:ring-2 focus:ring-offset-2  text-sm leading-none text-red-600 py-2 px-2 bg-gray-100 rounded hover:bg-gray-200 focus:outline-none"><AiTwotoneDelete /></button>
                                </div>
                            </td>
                        </tr>)
                }
