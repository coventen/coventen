...
mutation Mutation($input: [SolutionPageCreateInput!]!) {
    createSolutionPages(input: $input) {
      info {
        nodesCreated
      }
    }
...
mutation DeleteSolutionPages($where: SolutionPageWhere) {
    deleteSolutionPages(where: $where) {
      nodesDeleted
    }
  }
`
...
const Main = () => {
    ...
    const { data, loading, error, refetch } = useQuery(GET_SERVICES, { client })

    //MUTATIONS
    const [createServiceFn, createState] = useMutation(CREATE_SERVICE, { client })
    const [deleteServiceFn, deleteState] = useMutation(DELETE_SERVICE, { client })
    ...
    const createService = async (title: string) => {
        const { data } = await createServiceFn({
            variables: {
                input: {
                    title: title
                }
        ...
        if (data.createSolutionPages.info.nodesCreated > 0) {
    ...
    const deleteService = async (id: string) => {
        ...
        if (data.deleteSolutionPages.nodesDeleted > 0) {
    ...
    console.log(data?.solutionPages)
            ...
            <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div className="inline-block min-w-full  rounded-sm overflow-hidden">
                    ...
                    <table className="w-full leading-normal">
                        ...
                        <tbody>
                            {
                                data?.solutionPages && data?.solutionPages?.map((item: any, i: number) =>
                                    <tr key={i}>
                        ...
                        <CreateServiceModal isOpen={isModalOpen} setIsOpen={setIsModalOpen} createService={createService} />
                    </table>
                </div>
            </div>
