...
const DELETE_SUB = `
mutation Mutation($where: SubSolutionWhere) {
    deleteSubSolutions(where: $where)  {
      nodesDeleted
    }
  }
`
...
const Main = () => {
    ...
    const { data, loading, error, refetch } = useQuery(GET_SUB, { client })

    //MUTATIONS
    const [deleteServiceFn, deleteState] = useMutation(DELETE_SUB, { client })
    ...
    console.log(data?.subSolutions, 'kkkk')
    return (
        <>
            <div className='flex items-center justify-end'>
                <div>
                    <Link href='/admin/dashboard/settings/solution_page/sub_solution/create' className="focus:ring-2 focus:ring-offset-2 focus:ring-primary mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-primary hover:bg-primary focus:outline-none rounded">
            ...
            <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div className="inline-block min-w-full  rounded-sm overflow-hidden">
                    ...
                    <table className="w-full leading-normal">
                        ...
                        <tbody>
                            {
                                data?.subSolutions && data?.subSolutions?.map((item: any, i: number) =>
                                    <tr key={i}>
