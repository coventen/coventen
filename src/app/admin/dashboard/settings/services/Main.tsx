...
const Main = () => {
    ...
    const { data, loading, error, refetch } = useQuery(GET_SUB_SERVICES, {
        client,
        variables: {
            "where": {
                "isService": true,
    ...
    return (
        <>
            <div className='flex items-center justify-end'>
                <div>
                    <Link href='/admin/dashboard/settings/services/create' className="focus:ring-2 focus:ring-offset-2 focus:ring-primary mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-primary hover:bg-primary focus:outline-none rounded">
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-xs">
                                            <div className="relative flex items-center justify-center  space-x-4 px-8 ">
                                                <Link href={`/admin/dashboard/settings/services/details/${item?.id}`} className="focus:ring-2 focus:ring-offset-2  text-sm leading-none text-primary py-2 px-2 bg-primary/20 rounded  focus:outline-none">Update</Link>
