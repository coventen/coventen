...
const CommunicationPage = () => {
    ...
    // fetching messages
    const { data, loading, error, refetch } = useQuery(GET_SET_MESSAGES, { client })
                        ...
                        <ul className=''>
                            {
                                data?.communicationTickets.length && data?.communicationTickets?.map((item: any) =>
                                    ...
                                    <li key={item?.id} >
                                        <Link href={`/admin/dashboard/communication/message_preview/${item?.id}`}
                                            ...
                                            <div className=" flex items-center justify-between p-1 my-1 cursor-pointer  w-full">
                                                ...
                                                <div className=" flex items-center justify-end">
                                                    <div className="flex items-center space-x-2" >
                                                        ...
                                                        <button title="Clock">
                                                            <svg xmlns="http://www.w3.org/2000/svg" className="text-gray-500 hover:text-gray-900 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path strokeLinecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                            </svg>
                                                        </button>
                                                    </div>
