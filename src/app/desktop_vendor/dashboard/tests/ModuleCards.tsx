...
const ModuleCards = ({ data, deleteModuleById }: { data: Module[], deleteModuleById: any }) => {
    ...
    return (
        <div>
                                        ...
                                        <div className="">
                                            <div className="pb-10 relative">
                                                <h5 className="text-primaryText font-bold text-md  mb-2">
                                                    Id: {module?.ticket || 'Not Assigned Yet'}
                                                </h5>
                                        ...
                                        <div className='w-full flex items-center justify-end space-x-3'>
                                            {
                                                module.status === 'PENDING' ?
                                                    <button
                                                        ...
                                                        className='bg-green-200 mr-3 text-green-600 font-semibold text-xs px-3 py-1 rounded text-right'>
                                                        Assign
                                                    </button>
                                                    :
                                                    <button
                                                        className='bg-gray-200 mr-3 text-gray-700 font-semibold text-xs px-3 py-1 rounded text-right'>
                                                        Assigned
                                                    </button>
                                            }

                                            {
                                                !module?.testticketHas || module?.testticketHas?.status === "PENDING" ? <button
                                                    onClick={() => {
            ...
            <AssignmentModal isOpen={isOpen} setIsOpen={setIsOpen} currentModuleId={currentModuleId} />
        </div>
    );
};
