            ...
            {products.length ? (
              <>
                {products.map((product, index) => (
                  <div
                    key={index}
                    className="group relative my-10 flex w-full  flex-col overflow-hidden  border rounded dark:bg-darkBgLight dark:border-darkBorder bg-white/30 cursor-pointer "
                  >
                    <div className="relative mx-3 mt-3 flex  h-48 overflow-hidden rounded group">
                      <img
                        className="peer absolute top-0 right-0  h-48 w-full  rounded overflow-hidden transition-all duration-500 group-hover:scale-125"
                    ...
                    <div className="mt-4 px-5 pb-3 mb-12 ">
                      <h5 className="text-base text-gray-900 tracking-tight font-semibold uppercase mb-2">
                        {product?.title}
                      </h5>
                    </div>
                    <div className=" px-5 pb-5  absolute bottom-2 w-full">
                      <div className="flex items-center w-full mt-3 justify-between">
                        <span className="text-lg flex items-center justify-center space-x-2 font-semibold  dark:text-white  text-gray-900">
                          Price: {product?.price}
                        </span>

                        <Link
                          href={`/products/details/${product?.id}`}
                          className="text-white gradient-bg font-semibold rounded  text-sm px-4 py-1.5 text-center "
                        >
                          View Details
                        </Link>
                      </div>
